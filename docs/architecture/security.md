...
Agent command execution has a 300-second wall-clock timeout and captures at most one MiB from each output stream while continuing to drain the child. Agent sessions additionally enforce step, total observation bytes, failure, repeated-action, and 30-minute wall-clock budgets.
...
The desktop writes filesystem mutations, process and PTY lifecycle operations, Git mutations, patch and rollback actions, model-runtime lifecycle operations, and MCP operations to `.helel/audit.jsonl`. New append-only records use a SHA-256 hash chain; verification remains compatible with legacy developer-preview records and checks the complete chain before appending. Local model startup verifies a recorded checkpoint SHA-256 when `checkpoint.json` accompanies the selected weights. MCP is restricted to explicitly configured local stdio children, performs the MCP 2025-06-18 initialization lifecycle, discovers tools, and requires approval for every tool call. MCP descriptions and results are labeled untrusted when added to a model prompt.

Local voice input is a user-initiated, explicitly approved process action. The desktop accepts only a RIFF/WAVE payload up to 1.1 MB, invokes the configured local `whisper.cpp` executable with fixed shell-free arguments, enforces a 120-second timeout, limits transcript size, and removes the temporary recording and transcript after success or failure. Voice text is placed into the editable task field and never starts an agent automatically. Speech output uses the fixed macOS `/usr/bin/say` executable, has a 2,000-byte limit, and both voice operations extend the workspace audit chain.
