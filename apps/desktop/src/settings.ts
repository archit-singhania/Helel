...
export interface Settings { theme: Theme; sidebarWidth: number; agentWidth: number; bottomPanelHeight: number; recentProjects: string[]; modelProgram: string; modelArgs: string; modelConfig: string; modelTokenizer: string; modelWeights: string; }
export const SETTINGS_KEY = "helel.settings.v1";
export const DEFAULT_SETTINGS: Settings = { theme: "dark", sidebarWidth: 248, agentWidth: 320, bottomPanelHeight: 190, recentProjects: [], modelProgram: "helel-inference", modelArgs: "", modelConfig: "", modelTokenizer: "", modelWeights: "" };
...
export function parseSettings(value: string | null): Settings {
  ...
  try {
    ...
    return { theme: theme === "dark" || theme === "light" || theme === "system" ? theme : DEFAULT_SETTINGS.theme, sidebarWidth: bounded(candidate.sidebarWidth, DEFAULT_SETTINGS.sidebarWidth, 200, 420), agentWidth: bounded(candidate.agentWidth, DEFAULT_SETTINGS.agentWidth, 260, 520), bottomPanelHeight: bounded(candidate.bottomPanelHeight, DEFAULT_SETTINGS.bottomPanelHeight, 120, 420), recentProjects: Array.isArray(candidate.recentProjects) ? candidate.recentProjects.filter((path): path is string => typeof path === "string" && path.length > 0).slice(0, 8) : [], modelProgram: typeof candidate.modelProgram === "string" ? candidate.modelProgram : DEFAULT_SETTINGS.modelProgram, modelArgs: typeof candidate.modelArgs === "string" ? candidate.modelArgs : "", modelConfig: typeof candidate.modelConfig === "string" ? candidate.modelConfig : "", modelTokenizer: typeof candidate.modelTokenizer === "string" ? candidate.modelTokenizer : "", modelWeights: typeof candidate.modelWeights === "string" ? candidate.modelWeights : "" };
