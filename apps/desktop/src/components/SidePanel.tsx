...
export function SidePanel(props: Props) {
  ...
  return <aside className="side-panel" aria-label={title} style={{ width: props.width }}><p className="panel-title">{title}</p>
    ...
    {props.view === "source" && <div className="git-panel"><button className="git-refresh" onClick={props.onRefreshGit}>Refresh</button>{props.git ? <><p className="git-branch"><Icon name="branch" /> {props.git.branch}</p><div className="section-label">Changes · {props.git.changes.length}</div><ul>{props.git.changes.length ? props.git.changes.map((change) => <li key={change}>{change}</li>) : <li className="clean">Working tree clean</li>}</ul><div className="section-label">Unstaged diff</div><pre>{props.git.diff || "No unstaged changes"}</pre><div className="section-label">Structured patch</div><textarea value={patch} onChange={(event) => setPatch(event.target.value)} placeholder="Paste a unified Git patch to validate…" aria-label="Unified patch"/><div className="patch-actions"><button onClick={() => props.onApplyPatch(patch, false)}>Check & apply</button><button onClick={() => props.onApplyPatch(patch, true)}>Reverse</button></div></> : <p className="empty-copy">Open a Git repository to inspect source control.</p>}</div>}
